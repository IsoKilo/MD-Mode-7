...
; To further reduce the amount of projection tables, angles are limited to 16 6 degree steps.
...
; This brings the advantage of colour mixing with dithering by using the upper nybble of a map pixel as a dither pixel inside our double wide pixel,
; bumping the map colour range from 16 to an effective 256, matching the SNES.
...
; Manage the ROM banks the map data is read from. This can be done by setting mapper bank 7 to 7+(map_id/2).
...
; Test results 17/03/2025
; Extremely laggy and buggy, but the principles used are the right approach.
...
Mode7_DrawFloor:
        ...
        btst	#0,(map_id).w	                        ; We can fit 2 512x512 maps in a 512kb ROM bank, check if we should use the upper one. 
...
.loop:
		...
		bge.s	.outofbounds                            ; If so, draw out of bounds.
		asl.w	#8,d2								    ; Multiply by 512  (MAP_SIZE) to get row index.
		asl.w	d2
...
; --------------------------------------------------
    ...
    org $380000
		...
		incbin	"Map.bin"	; Super Mario Kart's Mario Circuit 4 example.
		; You can contiguously include the map data without orging back to $380000, we're at the back of the ROM anyways.
